...
//        std::cout << "Desc set. "   << query.value(3).toString().toStdString() << std::endl;
        // Store
        transaction->setStore(Store(query.value(4).toString()));
...
QList<QString> ReadTable::getMembers() {
    members.clear();
    QSqlQuery query;
    QString queryStr = QString("SELECT DISTINCT Member FROM transactions");
    query.prepare(queryStr);
    if ( !query.exec() ) { std::cout << "Query failed." << std::endl; }
    while ( query.next() ) {
...
QList<QString> ReadTable::getStores() {
    stores.clear();
    QSqlQuery query;
    QString queryStr = QString("SELECT DISTINCT Store FROM transactions");
    query.prepare(queryStr);
    if ( !query.exec() ) { std::cout << "Query failed." << std::endl; }
    while ( query.next() ) {
        stores.append(query.value(0).toString());
    }
    return stores;
}
